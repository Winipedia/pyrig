"""module."""

from pathlib import Path

from pyrig.dev.configs.issue_templates.config import ConfigConfigFile


class TestConfigConfigFile:
    """Test class."""

    def test_get_parent_path(self) -> None:
        """Test method."""
        result = ConfigConfigFile.get_parent_path()
        assert result == Path(".github/ISSUE_TEMPLATE")

    def test__get_configs(self) -> None:
        """Test method."""
        result = ConfigConfigFile._get_configs()  # noqa: SLF001
        assert isinstance(result, dict)

    def test_is_correct(self) -> None:
        """Test method."""
        assert ConfigConfigFile().is_correct()
