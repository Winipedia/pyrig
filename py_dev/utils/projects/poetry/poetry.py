...
from py_dev.utils.string import make_name_from_obj
...
def get_run_python_module_args(module: ModuleType) -> list[str]:
    ...
    from py_dev.utils.modules.module import (  # noqa: PLC0415  # avoid circular import
        make_obj_importpath,
    )
...
def get_poetry_run_py_dev_cli_cmd_args(
    cmd: Callable[[], Any] | None = None,
    extra_args: list[str] | None = None,
) -> list[str]:
    """Get the args to run py_dev."""
...
def get_poetry_run_cli_cmd_script(cmd: Callable[[], Any]) -> str:
    """Get the script to run py_dev."""
    return get_script_from_args(get_poetry_run_cli_cmd_args(cmd))


def get_poetry_run_py_dev_cli_cmd_script(cmd: Callable[[], Any]) -> str:
    """Get the script to run py_dev."""
    return get_script_from_args(get_poetry_run_py_dev_cli_cmd_args(cmd))
