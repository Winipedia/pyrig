...
No special permissions needed (publishing uses PyPI token, not GitHub permissions)
...
Runs on `ubuntu-latest` only if release workflow succeeded
...
Triggers after release workflow completes successfully, builds wheel, and publishes to PyPI.

### Manual Execution

Go to Actions → Publish → Run workflow (useful for republishing or testing)
