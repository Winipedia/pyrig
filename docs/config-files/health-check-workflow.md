...
- **Secret:** `CODECOV_TOKEN` (required for private repos, optional for public repos)
- **Failure behavior:** Workflow will fail if upload fails and token is configured
- **Why:** Tracks coverage trends over time

**Note:** For public repositories, Codecov can work without a token if you connect your GitHub account. For private repositories, you must configure `CODECOV_TOKEN`. The step will fail the workflow if the upload fails (`fail_ci_if_error: true`).
