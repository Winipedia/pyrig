...
- Includes all standard badges (tooling, testing, code quality, project info,
...
2. **Badge sections**: Seven categories of badges (tooling, testing, code
   quality, project info, security, CI/CD, documentation)
...
The `ReadmeConfigFile` inherits the `lines()` method from
...
Badges are organized into seven categories: tooling, testing, code quality,
project info, security, CI/CD, and documentation. See [Index.md](index_md.md)
for the complete badge list.
...
The validation (inherited from `BadgesMarkdownConfigFile.is_correct()`) checks
that the README file contains all lines generated by `lines()`.
