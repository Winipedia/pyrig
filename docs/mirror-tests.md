...
file and appending skeletons for any untested symbols. `_configs()` delegates to
`lines()`. `_load()` reads the existing test file. `validate()` writes the result
when the file is missing or incorrect — it never removes existing test implementations.
