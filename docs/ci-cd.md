...
CI** on the health check stage, not as a manual developer step. The health check
stage runs `uv lock --upgrade` to pull the latest dependency versions within
declared constraints. The release stage creates and pushes a version tag, then
publishes a GitHub Release with the build artifacts from the previous stage.
